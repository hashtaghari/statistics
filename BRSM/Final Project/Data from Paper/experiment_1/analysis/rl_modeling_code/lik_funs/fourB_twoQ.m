...
function [loglik] = fourB_twoQ(parameters, subj)
...
nd_Qinit_c = parameters(6); %normally distributed Qinit (transformed to be between -1 and 1)
q_init_c = nd_Qinit_c/10;

nd_Qinit_e = parameters(7); %normally distributed Qinit (transformed to be between -1 and 1)
q_init_e = nd_Qinit_e/10;
...
for trial_num = 1:T
    ...
    if trial_num == 1 || trial(trial_num) == 1 || trial(trial_num) < trial(trial_num - 1)
        exemp_ests = q_init_e * ones(1, 15);
        cat_ests = q_init_c * ones(1, 3);
    end
