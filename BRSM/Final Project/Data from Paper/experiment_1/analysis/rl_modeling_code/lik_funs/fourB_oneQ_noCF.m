...
function [loglik] = fourB_oneQ_noCF(parameters, subj)
...
for trial_num = 1:T
    ...
    if trial_num == 1 || trial(trial_num) == 1 || trial(trial_num) < trial(trial_num - 1)
        exemp_ests = Qinit * ones(1, 15);
        cat_ests = Qinit * ones(1, 3);
    end
    ...
    %decay all values to Qinit 
    %cat_ests = cat_ests + phi * (Qinit - cat_ests);
    %exemp_ests = exemp_ests + phi * (Qinit - exemp_ests);
